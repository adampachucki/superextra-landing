...
export function buildIntakePrompt({
	...
	return [
		...
		'Ask only when the missing detail is needed. If the user gives enough restaurant, address, area, or market scope, start research.',
		'Treat the user latest reply as accept, reject, repair, or new request before deciding scope. If it accepts or repeats a single scope you proposed in your previous reply, treat that scope as confirmed and start research.',
		'Act on the best high-confidence interpretation. State the interpretation in the acknowledgement when you start research. Use reply only when scope is genuinely missing or contradicted, not to confirm a value the user has just supplied or echoed.',
		'When your previous reply offered multiple alternatives, a plain yes is not confirmation. Only commit when the user names one alternative, picks by number, or otherwise selects.',
		'Do not repeat a question you have already asked in a different shape unless the user has rejected your prior proposal. If two attempts to fill the same slot have failed and a usable named scope or proposed correction is available, start research with that as your best assumption and state it in the acknowledgement.',
		'First set scopeKind to the model-owned semantic role of the latest usable scope:',
		'- research_scope: enough named geography, market, catchment, or non-food-service landmark-as-geographic-shorthand to research directly in search queries without resolving a Place ID. Broad industry questions need a named geography or an explicit national or global frame; otherwise treat as insufficient_scope.',
		...
		'A clear matching candidate must match, normalize, or obviously correct the requested establishment or brand name (one-character typo, accent or transliteration difference, brand variant) and match the important location cues. Do not select a candidate just because the street, neighborhood, or city matches if its name is a different establishment.',
