...
// ─────────────────────────────────────────────────────────────────────
// R3.2 — DIAGNOSTIC FUNCTIONS — TEMPORARY
// To be removed via `firebase functions:delete probeHandoffAbort
// probeHandoffLeaveOpen --region us-central1 --project=superextra-site --force`
// after R3 results are captured. See docs/gear-probe-plan-round3-2026-04-26.md.
// ─────────────────────────────────────────────────────────────────────

import { GoogleAuth } from 'google-auth-library';

const LIFECYCLE_RESOURCE =
	'projects/907466498524/locations/us-central1/reasoningEngines/329317476414259200';
const STREAM_LOCATION = 'us-central1';

async function _streamQueryToken() {
...
async function _readFirstNdjsonLine(stream) {
	...
	try {
		while (true) {
			...
			if (done) {
				return { line: null, reader };
			}
			buffer += decoder.decode(value, { stream: true });
			const newlineIdx = buffer.indexOf('\n');
			if (newlineIdx >= 0) {
				const line = buffer.slice(0, newlineIdx).trim();
				if (line) return { line, reader };
				buffer = buffer.slice(newlineIdx + 1);
			}
		}
	} catch (err) {
		return { line: null, reader, error: String(err) };
	}
}

async function _writeCfReturnedAtMarker(sid, variant) {
	await db.collection('probe_runs').doc(sid).collection('markers').doc('cf_returned_at').set({
...
async function _runHandoff(req, res, { abortAfterFirstLine }) {
	if (req.method !== 'POST') {
		res.status(405).json({ ok: false, error: 'POST only' });
		return;
	}
	const sid = (req.body && req.body.sessionId) || null;
	if (!sid) {
		res.status(400).json({ ok: false, error: 'missing sessionId in body' });
		return;
	}
	const variant = abortAfterFirstLine ? 'abort' : 'leave_open';
	console.log(`[probeHandoff:${variant}] start sid=${sid}`);

	const token = await _streamQueryToken();
	const url = `https://${STREAM_LOCATION}-aiplatform.googleapis.com/v1/${LIFECYCLE_RESOURCE}:streamQuery?alt=sse`;
	...
	const fetchRes = await fetch(url, {
		...
		body: JSON.stringify({
			...
			input: { user_id: `r32-${variant}`, session_id: sid, message: 'go' }
		})
	});
	console.log(
		`[probeHandoff:${variant}] fetch status=${fetchRes.status} elapsed=${Date.now() - t0}ms`
	);

	if (!fetchRes.ok) {
		const body = await fetchRes.text().catch(() => '');
		res.status(502).json({ ok: false, status: fetchRes.status, body: body.slice(0, 500) });
		return;
	}

	const { line, reader, error } = await _readFirstNdjsonLine(fetchRes.body);
	console.log(
		`[probeHandoff:${variant}] firstLine=${line ? line.slice(0, 120) : '(none)'} error=${error ?? 'none'}`
	);

	await _writeCfReturnedAtMarker(sid, variant);

	if (abortAfterFirstLine) {
		// EXPLICIT clean disconnect — gate variant
		try {
			await reader.cancel();
		} catch (e) {
			console.log(`[probeHandoff:abort] reader.cancel() error: ${e}`);
		}
		try {
			controller.abort();
		} catch (e) {
			console.log(`[probeHandoff:abort] controller.abort() error: ${e}`);
		}
		console.log(`[probeHandoff:abort] disconnected; returning 202`);
	} else {
		// Leave-open diagnostic — DOES NOT cancel reader or abort fetch.
		// Background fetch lifetime undefined per Firebase docs.
		console.log(`[probeHandoff:leave_open] returning 202 with fetch still pending`);
	}

	res.status(202).json({
		handoff: line ? 'received_first_event' : 'no_first_event',
		variant,
		first_line_len: line?.length ?? 0
	});
}

export const probeHandoffAbort = onRequest(
	// timeoutSeconds=90 (vs production agentStream's 30s) — required to
	// give the lifecycle DeterministicSlowAgent's 60s first-event window
	// room. Migration finding: production agentStream rewrite must bump
	// to similar to wait for first-NDJSON-line handoff proof.
	{ cors: true, timeoutSeconds: 90 },
	async (req, res) => _runHandoff(req, res, { abortAfterFirstLine: true })
);

export const probeHandoffLeaveOpen = onRequest(
	{ cors: true, timeoutSeconds: 90 },
	async (req, res) => _runHandoff(req, res, { abortAfterFirstLine: false })
);
