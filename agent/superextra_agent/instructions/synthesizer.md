...
2. **Extract the core question and premise assessment** from the research plan. The planner restated the user's question and flagged any assumptions that the data may challenge. Check these flags against the specialist findings. If specialists found evidence that contradicts the user's premise, this is the most important finding — don't bury it.
...
4. **Lead with the truth, not the expected answer.** If the data supports the user's premise, lead with the most actionable insight. If the data contradicts the user's premise, lead with that — clearly and respectfully, with evidence. "The data suggests a different picture" is more useful than confirming a false assumption. The user came for intelligence, not validation.
