...
`get_tripadvisor_reviews(url, max_reviews=50, mode="fast")`

Fetches TripAdvisor reviews from a Restaurant_Review page URL. Default fast mode can return up to 100 reviews via SerpAPI. Use the sampling policy below for review analysis. Use `mode="deep"` or `max_reviews > 100` only for owner-response analysis, deeper history, subratings, or place histogram; deep mode returns up to 300 reviews and is slower.
...
2. For each requested place, call `get_google_place_signals(place_id, max_reviews=50)`. Use the returned Google review sample, rating distribution, review themes, and subratings for quantitative analysis.
