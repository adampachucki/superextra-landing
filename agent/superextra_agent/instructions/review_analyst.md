...
- In `Evidence Notes`, cite provider data with stable references such as `Google Reviews place_id:<Google Place ID>` or `TripAdvisor place_id:<TripAdvisor place ID>`. Do not invent URLs; include a TripAdvisor URL only when the verified profile result returned one.
