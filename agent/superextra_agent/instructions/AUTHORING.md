...
Router → research_pipeline
           ...
           ├── Research Orchestrator (reconnaissance, plans, calls specialists as tools)
...
- Specialist result keys (`market_result`, `pricing_result`, etc.) — written by specialists when the orchestrator calls them as tools.

**Agent roles:**

- **Router** detects conversation state and routes.
- **Orchestrator** does reconnaissance, audits assumptions, defines the competitive set, and calls specialists directly as tools with specific `request` briefs.
...
2. **Agents don't coordinate.** If scopes overlap, they produce identical output. The orchestrator must call specialists with non-overlapping briefs and explicit boundaries. The orchestrator also defines a shared competitive set so all specialists analyze the same restaurants.
...
4. **Specialists can't infer what they can't see.** Each specialist `request` must relay: response language, date, competitive set, what other specialists are covering.
...
2. Add a specialist entry to `specialist_catalog.py` with `dispatchable=True`
3. Add a tool override in `specialists.py` only if the specialist needs non-default tools
4. Add `{result_key}` in `synthesizer.md`
