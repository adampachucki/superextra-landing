...
The router routes. The context enricher builds Google Places context. The Research Lead plans, briefs specialists, checks sufficiency, and records internal research coverage notes. Specialists answer one evidence surface from the Lead's brief and surface as much useful writer material as possible. Public-web specialists use native Google Search grounding and URL Context by default; source pills come from grounding/provider provenance. Custom search/read tools are reserved for dedicated narrow agents where explicit URL discovery or raw page fetching is the product. Each specialist owns the source-quality notes for its evidence surface. The Report Writer reads Places context and specialist reports directly, without a lead-authored brief, and writes the final report. Continue Research answers from the existing report, specialist notes, Places context, direct venue checks, known-URL checks through focused helpers, and bounded non-durable specialist deepening. Source discovery belongs in focused helpers, not direct continuation-agent search. Broad new reports should become new research sessions instead of being merged into the old thread.
...
| Specialist source reading, evidence limits, and source-quality notes | `specialist_base.md` |
| Final report synthesis, report shape, charts, user-facing depth, and source-limit handling | `report_writer.md` |
...
1. `specialist_base.md`: universal specialist contract, source-reading expectations, evidence notes, and substantial evidence report shape.
...
The Lead should use at least two specialists for every research report. A dynamic researcher can count when its brief owns a distinct deeper angle or evidence surface. 3-5 total specialists is common. Add another specialist when it gives a useful perspective, test, or evidence surface. Dynamic researchers are favored for substantive operator research when there is a concrete deeper angle: causes, relationships, implications, conflicts, second-order effects, verification, or gap-filling. Skip the dynamic pass when the first specialist round already gives a clear, well-supported answer and another pass would mostly repeat or decorate. Do not let a dynamic researcher replace a core evidence surface.
