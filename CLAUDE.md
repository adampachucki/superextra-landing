...
- **Exported wordmark/tagline are outlined SF Pro Light**, not live text. The on-page wordmark is CSS `font-weight: 300` over `-apple-system, …, 'Inter'` — which is only Light where SF Pro resolves (Apple); the Inter webfont must include the 300 face (loaded in `app.html`) or it falls back to Regular. To make every SVG/PNG export carry true SF Pro Light **font-independently** (any viewer, no SF Pro needed), `scripts/build-brand-glyphs.py` instances SFNS at the Light wght (`274.315`) and writes vector outlines → `src/lib/brand/brand-glyphs.js` (committed; the route's export engine renders these paths). Regenerate only when the wordmark/tagline strings or font change (needs SF Pro, macOS): `python3 scripts/build-brand-glyphs.py`. Note: using SF Pro in redistributed brand assets is outside Apple's font license — owner-accepted.
