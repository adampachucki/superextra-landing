<script lang="ts">
	...
	// Track prompt bar height for bottom padding on chat thread
	$effect(() => {
		if (!promptBarEl) return;
		const ro = new ResizeObserver((entries) => {
			promptBarHeight = entries[0].contentRect.height;
		});
		ro.observe(promptBarEl);
		return () => ro.disconnect();
	});

	// Track mobile keyboard via visualViewport so the prompt bar stays above the keyboard
	$effect(() => {
...
<div class="chat-enter relative flex min-h-dvh {mounted ? 'is-mounted' : ''}">
	...
	<div
		...
		style="padding-bottom: {promptBarHeight + 8}px"
	>
		<!-- Chat thread -->
		{#if chatState.active}
			<div
...
<!-- Input bar (outside chat-enter, no animation transform) -->
<!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
<div
	...
	sidebarOpen
		? 'lg:left-64'
		: ''}"
	style={keyboardOffset > 0 ? `bottom: ${keyboardOffset}px` : ''}
>
	<div
