...
- **Firestore query shape MUST be `collectionGroup(db, 'events')` with `where('userId','==',uid).where('runId','==',runId).orderBy('attempt').orderBy('seqInAttempt')`.** Subcollection queries on `sessions/{sid}/events` with the same filters require a COLLECTION-scoped composite index (which can be created via gcloud or `firestore.indexes.json`, contrary to an earlier assertion of mine — see finding D.3). The collection-group query is a deliberate simplification: one unified index + rule shape across all sessions. Spike D finding D.1.
...
**Finding D.2** — `onSnapshot` delivers full current state in one callback (20/20 docs), in order `(attempt ASC, seqInAttempt ASC)`. Refresh-mid-pipeline recovery path confirmed feasible. **Anecdotal observation (not a documented Firestore contract)**: the first listener subscription transiently returned a FAILED_PRECONDITION that resolved after ~30 s without me doing anything — possibly index propagation settling, possibly an auto-managed helper index. I cannot confirm which from Firestore docs. Implementers should plan for: on a fresh deploy, expect a small window where listeners may need one auto-retry before delivering data. Don't rely on any specific behaviour here.
