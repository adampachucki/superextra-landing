...
- **Quality side-by-side, gear vs cloudrun (2026-04-27, post-merge).** Same prompt + same place context, fresh anon UIDs, runs 5 min apart through GHA-deployed `agentstream` revision 56:
  - gear sid `907c3bd4-9da0-4c6b-9cbe-b49ee8b25453`: 6058 chars, 32 sources, title `'Brussels Brasserie Pricing Positioning'`, ~5 min runtime
  - cloudrun sid `5edfe9eb-4e01-4ab0-a1ce-d6d6d099e744`: 6393 chars, 13 sources, title `'Le Vintage Brasserie Comparison'`, ~167 s runtime

  Both replies have parallel structure (executive summary → pricing benchmarks → signature dishes → market positioning → follow-up questions), both embed a chart, both nail Le Vintage's "trendy hybrid" positioning vs. the traditional brasserie set, both have concrete euro figures (€24-€36 mains) and concrete competitor analysis. Differences: gear named PLOEGMANS / Brasserie de Bruxelles / Mazette; cloudrun named PLOEGMANS / Le Bistro - Porte de Hal / Brasserie de Bruxelles — both reasonable competitor picks (research orchestrator nondeterminism). Source counts differ (32 vs 13) because gear surfaces Vertex grounding redirects (`vertexaisearch.cloud.google.com` × 29) inline; cloudrun stays with the SerpAPI/Apify/Maps mix.

  **Verdict:** gear quality is comparable to cloudrun on this prompt. The migration did not degrade the user-facing answer. Replies saved at `/tmp/gear_reply.md` and `/tmp/cloudrun_reply2.md` for offline diff.

  **Bug found during the drill:** the first cloudrun submission (sid `3caa8c9a-...`, before the unblock fix) failed in 10 s with `error='empty_or_malformed_reply'`. Worker logs traced it to `FirestoreProgressPlugin.before_run_callback` returning a halt-content because `runId` was missing from `session.state` — and the legacy worker doesn't set `runId` into ADK state. The plugin was registered globally on the shared `App`, so it killed every worker run since the gear migration landed. Fix shipped in `2e618b6` (PR #12): plugin no-ops when state is missing instead of halting. The retest above (sid `5edfe9eb-...`) is the post-fix verification.
