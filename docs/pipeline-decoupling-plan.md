...
**Why collection-group query (deliberate design choice)**: spike D verified that `COLLECTION_GROUP`-scoped composite indexes do not cover path-scoped subcollection queries. We'd need either a `COLLECTION`-scoped index (which `gcloud firestore indexes composite create --query-scope=collection` and `firebase deploy --only firestore:indexes` both support — the CLI is not the limiter) or a collection-group query. We chose the collection-group query because it gives a single unified index + rule shape that works across all sessions without per-parent-path ceremony: one composite index on `(userId, runId, attempt, seqInAttempt)` serves every user's every turn, and one rule (`match /{path=**}/events/{eid}`) governs reads. The client uses `collectionGroup('events').where('userId','==',uid).where('runId','==',runId).orderBy(...)`.
...
1. **Auth is handled by Cloud Run IAM — no in-handler token verification.** Cloud Tasks mints an OIDC token using the configured `serviceAccountEmail` (the worker SA). Cloud Run validates the token and checks the principal has `roles/run.invoker` before forwarding to the handler. Only the worker SA has `run.invoker` on this service, and only Cloud Tasks can impersonate the worker SA (via the pair `roles/iam.serviceAccountUser` + `roles/iam.serviceAccountTokenCreator` granted to the Cloud Tasks service agent — see Phase 6). A request that reaches the handler is therefore already authenticated as an OIDC-signed Cloud Tasks dispatch. Read `X-CloudTasks-TaskName` (= `runId`), `X-CloudTasks-TaskRetryCount`, `X-CloudTasks-TaskExecutionCount` as informational task metadata for logging and idempotency.

2. **Idempotent takeover / active-owner wait** — atomic Firestore transaction on `sessions/{sid}`:
   - Read existing doc.
   - **Defense-in-depth ownership assertion**: verify `existing.userId == payload.userId`. If not, abort (fatal error, return 500 — agentStream should have prevented this). Belt-and-suspenders on the identity that agentStream already validated.
   - **Stale-run guard** (critical — prevents an old Cloud Tasks redelivery from clobbering a newer turn): if `currentRunId != runId` AND `currentRunId is not None`, the session has moved to a different (newer) `runId` while this delivery was queued. Return `200` with `action: 'noop_stale'` — let Cloud Tasks mark the old task done and do not touch the session. Without this guard, an old redelivery for run A arriving after the user started run B will overwrite `currentRunId`, `status`, and `currentAttempt` back to run A's values, corrupting the newer turn.
   ...
   - Otherwise (including `status='queued'` on first delivery): set `status='running'`, `currentAttempt=(existing.get('currentAttempt') or 0) + 1` (agentStream resets to 0 on new turn; in-run retries increment), `currentWorkerId=uuid4()` (fresh random UUID per worker invocation — unique fencing identifier, no need to stringify Cloud Tasks headers), `lastHeartbeat=now`, `lastEventAt=now`. Commit. Note: we do NOT rewrite `currentRunId` here — the stale-run guard above ensures we only reach this branch when `currentRunId == runId` already.

   **Poll loop** for active-owner case: every 5 s re-read the session doc. Exit conditions: `status` becomes terminal (return `200` — the other worker succeeded or failed cleanly, or the watchdog flipped it to `error`); `currentRunId` changes to a different value (session advanced mid-poll — return `200` with `noop_stale`); heartbeat goes stale (take over via a fresh takeover transaction); 7 min elapsed (raise `WorkerBusyError` → FastAPI returns 500 → Cloud Tasks backoff-retry). 7 min aligns with the watchdog's worst-case detection window — by then the watchdog will have classified the stale-pipeline case and B will observe a terminal state rather than timing out.
