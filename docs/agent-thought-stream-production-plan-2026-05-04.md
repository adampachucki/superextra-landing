...
| 5   | **Char-level streaming of thoughts** — Mike forwards every Gemini stream chunk's thought text to SSE without buffering (`gemini.ts:79-108`). Effect: thoughts type themselves character-by-character. | `gemini.ts:79-108`, `chatTools.ts:2425-2429`                   | **Yes, for thought rows only.** ADK's `StreamingResponseAggregator` (`utils/streaming_utils.py:266-297`) emits `partial=True` events per chunk. We stream only partial `kind: 'thought'` rows, ignore non-thought partial rows, and suppress the duplicate final aggregated thought row.                                      |

### 2.4 Things ADK provides that we should _consider_ using (verified, simplifies our code)

| Find                                                                                                       | What it replaces                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Cost                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. Stream partial thought rows only**                                                                    | `Event` extends `LlmResponse`; `LlmResponse.partial: Optional[bool]` lives at `models/llm_response.py:71`. Each thought arriving via the aggregator (`utils/streaming_utils.py:266-297`) can reach us before the final aggregated event (`partial=False`, yielded from `streaming_utils.py:340-367`).                                                                                                                                                                                                                                                                                                                                                                              | **Do NOT early-return on partial.** That would delay visible thought text and still skip the `lastEventAt` bump if done incorrectly. Instead, map partial events, keep only `kind: 'thought'` rows, bump `lastEventAt` for every event, and suppress the duplicate final aggregated thought row. Non-thought progress still comes from typed tool callbacks and final events. |
...
Replace the old progress wrapper rendering with the Step-blocks rendering from the preview. Same data, same types.
...
- `src/lib/components/agent/LiveActivity.svelte`
  ...
  - Render: preview-style card header with derived label + animated dots + elapsed timer, then numbered circle + vertical guideline + step title + thoughts (markdown via `renderMarkdown`) + muted nested tool rows.
...
**A. Stream partial thought rows only** (§2.4 A)

- `agent/superextra_agent/firestore_progress.py:on_event_callback` — call `observe_event(event)` for partial and final events. For `partial=True`, keep only `kind: 'thought'` rows for Firestore writes and mark that a partial thought is pending. For the next final aggregated event, drop its `kind: 'thought'` row so the user does not see the same thought repeated.
- Leave the `fenced_session_update({lastEventAt: SERVER_TIMESTAMP})` bump outside any partial/final branch — it must fire for every event so watchdog liveness holds.
- **Why:** first thought text appears as soon as ADK streams it, without exposing partial tool/source/terminal state.
- **Test:** a synthetic `event.partial=True` event writes only thought rows and bumps `lastEventAt`; a following `partial=False` event suppresses the duplicate thought row but still writes non-thought rows and bumps `lastEventAt`.
...
So the moment the turn ends, the live activity component unmounts and `liveTimeline` empties — there's no DOM left to show "Completed in N steps". Adding completed-state props to the old wrapper accomplished nothing visible.
...
#      - thought text appears before the final model event when ADK streams partial thoughts
...
```python
async def test_partial_event_streams_thought_only_and_bumps_lastEventAt(...):
    """Partial events may stream thought rows, but must bump lastEventAt
    for watchdog liveness."""
    # event with partial=True →
    #   GearRunState.observe_event called
    #   timeline_writer.write_timeline called only for kind='thought'
...
- `RESOLVED` Production uses the preview's derived label (`event.author` → friendly name + latest tool family) with animated dots and the elapsed timer.
- `RISK` `event.id` is regenerated per partial event (`event.py:77-81`). The frontend buffers consecutive thought rows before rendering Step blocks, and the backend suppresses the final duplicate aggregated thought row, but partial thought streaming still creates more timeline documents than final-only mode.
