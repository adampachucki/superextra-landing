...
| ------------------------------------------ | ---------------- | ---------------------------- | ----------- | ---------------------------------------------------------------------------------------------- | ------------------------------------- | ---------- |
| US Traffic — restaurants                   | `52549482610363` | Traffic · Landing Page Views | 100 PLN/day | US, 25–64, FB+IG (no Audience Network)                                                         | **PAUSED 2026-06-26**                 | 2026-06-23 |
| US Leads — restaurants                     | `52548746445163` | Leads (website)              | 100 PLN/day | US broad                                                                                       | **PAUSED** (cold pixel → no delivery) | 2026-06-22 |
| US Operators — restaurants (Traffic · LPV) | `52553900564763` | Traffic · Landing Page Views | 30 PLN/day  | US (living-in), 25+, **Food & Restaurant page admins**, Advantage+ off, FB + IG Feed · A/B/C/D | **LIVE 2026-06-26** (Preparing)       | 2026-06-26 |
...
- **Created:** 2026-06-26 · **Status:** **LIVE** — launched 2026-06-26 (Preparing → delivery; normal new-account first-spend hold). Old broad Campaign 2 paused the same day.
...
- **Ad set** `52553900768963` "Operators — Page Admins + restaurant interests · FB Feed · 25+ · LPV":
  ...
  - Targeting (live): **US (people living in) · age 25+ · Advantage+ OFF · Behaviors → Food & Restaurant page admins · Facebook + Instagram Feed** (IG runs under the Superextra Page identity — no IG account attached).
  - **✅ Operator targeting added in Ads Manager (2026-06-26):** the single behavior **Food & Restaurant page admins** (= people who run a restaurant's FB page) — tighter than the generic "Facebook Page admins" + interest-narrowing originally planned. This is the entire point; without it the ad set ≈ Campaign 2's broad set.
  - **Location-type gotcha (#1870178):** the saved audience carried a removed location option ("living in / recently in / traveling in") and the UI no longer exposes the selector to fix it. Fix: reset the ad set geo to US `location_types=["home"]` via API, then switch the ad set off the saved audience to a fresh inline audience.
- **Ads (4, all active):** A `52553900785163` (creative `1391486929494205`), B `52554265093963` (`1024668910101372`), C `52554265144963` (`1642272480166072`), D `52554265188563` (`1136322958925169`). Started A-only (the Campaign 2 winner), then restored the full A/B/C/D set — the old per-creative results were on the wrong audience, so let the new operator audience pick its own winner.
...
| --------------------- | ----- | ----- | ------ | --- | ----- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-26 (launched) | —     | —     | —      | —   | —     | —              | Live, "Preparing" (first-spend hold). 4 ads A/B/C/D · US living-in · Food & Restaurant page admins · FB+IG Feed. Fill numbers once delivery starts. |
...
- **2026-06-26** — **Campaign 3 LIVE; EngagedVisit deployed; Campaign 2 paused.** Deployed the EngagedVisit pixel event to prod (verified live: `init → PageView → EngagedVisit`; seeded ~14 events). Completed operator targeting in Ads Manager (**Food & Restaurant page admins**); fixed removed-location-type error #1870178 (reset geo to US "living in" via API + switched off the poisoned saved audience); added **Instagram Feed** (Page identity). Restored **all 4 ads A/B/C/D** for the new audience. **Launched** US Operators (`52553900564763`); **paused** old broad US Traffic — restaurants (final: 339.08 PLN, 6,875 impr, 269 clicks, 3.91% CTR, **0 prompt submissions**). EngagedVisit custom-conversion creation is **blocked until Meta registers the event** (not yet in the Events Manager picker — retry in a few hours), then switch the Operators goal once it fires ~30–50/wk.
